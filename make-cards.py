...
/* A landscape sheet flipped about its long edge inverts the vertical axis, so
   a back printed upright lands upside down on the cut card. Only the picture
   is turned, leaving the header upright on the uncut sheet where it is what
   you check the alignment against. The rotation is inside the cell, so it
   moves nothing: which card backs which is set by the grid position alone. */
...
.shot img {{ max-width: 100%; max-height: 100%; object-fit: contain; border: 0.2mm solid #ddd; transform: rotate(180deg); }}
