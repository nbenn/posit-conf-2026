...
::: {.notes}
<!-- What carries over to your next app? — ~74 s (176 words) -->
...
Now how can we improve? How can we have components carry over from one app to
the next?
...
**(1) On the left,** a graphical overview of the board, showing blocks, their
connections and stack membership.

**(2) On the right,** an AI assistant, which can manipulate and interrogate our
board.

Most blocks here are general purpose: reading data, transforming it, displaying
summary tables and visualizations that support drill-down.

**(3) But one block** was built specifically for this use case: the patient
profile you can see in the middle.
:::
...
::: {.notes}
<!-- Specialty pieces — ~24 s (59 words) -->
...
a blockr app. Here is what one looks like.
:::

## Your own block {transition="slide"}

```{.r code-line-numbers="1-4|6-7|8-9|13"}
new_head_block <- function(n = 6, ...) {
  ui <- function(id) {
    numericInput(NS(id, "n"), "Rows", n)
  }

  server <- function(id, x) {
    moduleServer(id, function(input, output, session) {
      expr <- reactive(bquote(utils::head(x, .(input$n))))
      list(expr = expr, state = list(n = reactive(input$n)))
    })
  }

  new_transform_block(server, ui, ...)
}
```

::: {.notes}
<!-- Your own block — ~32 s (76 words, 4 reveals) -->

The smallest block we ship, so you can see the stud with nothing in the way:
it takes the first rows of a table.

**(1-4) The user parameter** is one number, and the constructor argument seeds
it.

**(6-7) Data inputs** are arguments to what is otherwise a Shiny module. A join
block takes `x` and `y`.

**(8-9) The data output** is an expression, so the result is computed on demand
from whatever comes in.

**(13) Hand both to the constructor.** Communication and reactivity are taken
care of.

Real blocks put logic between input and output. The contract does not change.
:::
...
Unfortunately we don't get a Q&A. So if you have questions for me, want to see
or hear more, come find me.
:::
