...
format:
  revealjs:
    ...
    # crowding the closing slide, which is the one people photograph. BMS
    # leads so its own lockup — pink hand plus wordmark — stays contiguous:
    # with the square first it abutted the hand, and the three marks read as
    # one composite. The two coloured marks now bracket the row instead.
    footer: |
      <img src="img/bms.png" class="logo-bms"><span class="foot-sep"></span><img src="img/cynkra.png" class="logo-cynkra">
---
...
::: {.notes}
<!-- Any shape you want — ~18 s (40 words) -->
...
::: {.notes}
<!-- What carries over to your next app? — ~78 s (176 words) -->
...
**(1) On the left,** we have a graphical overview, showing blocks, their
connections and stack membership.

**(2) And on the right,** an AI assistant, which can manipulate and interrogate
our app.
...
::: {.notes}
<!-- Specialty pieces — ~26 s (58 words) -->
...
a blockr app. Here's what one looks like.
:::

## Anatomy of a block {transition="slide"}

```{.r code-line-numbers="|3|1|6|9|10"}
...
::: {.notes}
<!-- Anatomy of a block — ~31 s (69 words, 5 reveals) -->
...
**(4) The data output** is represented by an expression, so the result can be
computed on demand from input data.
